...
from uarm.srv import ConvertToJoints, ConvertToJointsResponse
...
class ROSKTHUarmBridge(object):
    ...
    def handle_convert_to_joints(self, request):
        """Process a cartesian to joint space request"""
        with self._uarm_lock:
            config = self._uarm.inverse_kinematics(request.position.x,request.position.y,request.position.z,request.eef_orientation,request.check_limits)
            response = ConvertToJointsResponse()
            [response.j0, response.j1, response.j2, response.j3] = config
            return response
...
if __name__ == '__main__':
    ...
    rospy.Service("uarm/convert_to_joints", ConvertToJoints, uarm_bridge.handle_convert_to_joints)
